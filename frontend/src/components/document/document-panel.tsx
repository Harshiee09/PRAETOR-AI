...
import { ACCEPT_ATTRIBUTE } from "@/lib/api/documents";
...
export function DocumentPanel({ label, document, disabled, uploading, now, onUpload, onView, onForget }: Props) {
  ...
  return <section className="document-panel" data-document={label} aria-labelledby={id}>
    ...
    {uploading ? <div className="document-uploading" role="status"><LoaderCircle className="spin" size={22} aria-hidden="true"/><strong>Reading your file…</strong><span>{uploading}</span><span className="document-uploading-note">Scans and photos are read with OCR and take a few seconds more.</span></div> : document ? <>
      ...
      <Upload size={25} aria-hidden="true"/><p>Upload a PDF, a Word document or a photo of a page (up to 4 MB). Scans and photos are read with OCR. It is kept for 60 minutes and never stored.</p>
      <input ref={input} className="sr-only" type="file" accept={ACCEPT_ATTRIBUTE} id={`${id}-file`} aria-label={`Upload Document ${label}`} disabled={disabled} onChange={(event) => { const file = event.target.files?.[0]; if (file) onUpload(file); event.target.value = ""; }}/>
      <button type="button" className="secondary-button" disabled={disabled} onClick={() => input.current?.click()}>Choose file{" "}<span className="sr-only">for Document {label}</span></button><span className="document-drop-hint">or drop it here</span>
    </div>}
  </section>;
}
