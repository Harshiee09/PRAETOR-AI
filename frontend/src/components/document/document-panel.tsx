...
export function DocumentPanel({ label, document, disabled, uploading, now, onUpload, onView, onForget }: Props) {
  ...
  return <section className="document-panel" data-document={label} aria-labelledby={id}>
    ...
    {uploading ? <div className="document-uploading" role="status"><LoaderCircle className="spin" size={22} aria-hidden="true"/><strong>Reading PDF…</strong><span>{uploading}</span></div> : document ? <>
      ...
      <Upload size={25} aria-hidden="true"/><p>Upload a PDF with selectable text (up to 4 MB). It is kept for 60 minutes and never stored.</p>
