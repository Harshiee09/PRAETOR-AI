import { isSupportedFile, MAX_PDF_BYTES, validatePdfFile } from "@/lib/api/documents";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (lengthHeader && (!/^\d+$/.test(lengthHeader) || Number(lengthHeader) > MAX_MULTIPART_BYTES)) return errorResponse("too_large", "Upload a file up to 4 MB.", requestId, 413);
  if (!request.body || !request.headers.get("Content-Type")?.toLowerCase().startsWith("multipart/form-data")) return errorResponse("invalid_request", "Choose a file to upload.", requestId, 422);
  let bytes = 0;
  let form: FormData;
  ...
  try {
    ...
    while (true) {
      ...
      if (bytes > MAX_MULTIPART_BYTES) return errorResponse("too_large", "Upload a file up to 4 MB.", requestId, 413);
    ...
    return errorResponse("invalid_request", "The file upload could not be read. Choose the file again.", requestId, 422);
  } finally {
    reader.releaseLock();
  }
  const file = form.get("file");
  if (!file || typeof file === "string" || form.getAll("file").length !== 1) return errorResponse("invalid_request", "Choose one file to upload.", requestId, 422);
  const invalid = validatePdfFile(file);
  if (invalid) {
    const status = !isSupportedFile(file) ? 415 : file.size > MAX_PDF_BYTES ? 413 : 422;
